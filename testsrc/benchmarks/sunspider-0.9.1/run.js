...
var repeatCount = 50; // number of iterations
var count = 25; // number of last iterations to consider
...
for (var currentRepeat = -1; currentRepeat < repeatCount; currentRepeat++) {
    ...
    for (var p in results.categories) {
        ...
        for (var q in category.tests) {
            ...
            var start = new Date();
            load(TEST_DIR + '/' + category.name + "-" + test.name + ".js");
...
timeMean(results.times).toString();
