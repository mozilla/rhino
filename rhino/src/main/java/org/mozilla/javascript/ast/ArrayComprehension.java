...
public class ArrayComprehension extends Scope {
    ...
    protected Node shallowCopy() {
        if (getClass() != ArrayComprehension.class) {
            throw new UnsupportedOperationException(
                    "shallowCopy() not implemented for " + getClass().getName());
        }
        ArrayComprehension copy = new ArrayComprehension();
        copy.type = this.type;
    ...
    protected void cloneNamedChildren(Node copyNode, IdentityHashMap<Node, Node> map) {
        ...
        ArrayComprehension copy = (ArrayComprehension) copyNode;
        if (this.result != null) {
            copy.result = (AstNode) this.result.cloneStructure(map);
        }
        if (this.loops != null) {
            List<ArrayComprehensionLoop> list = new ArrayList<>(this.loops.size());
            for (ArrayComprehensionLoop l : this.loops) {
                list.add((ArrayComprehensionLoop) l.cloneStructure(map));
            }
            copy.loops = list;
        }
        if (this.filter != null) {
