...
public class NativeRegExp extends IdScriptableObject {
    ...
    private static boolean parseDisjunction(CompilerState state, ParserParameters params) {
        ...
        if (index != source.length && source[index] == '|') {
             ...
             */
            if (result.kid.op == REOP_FLAT && result.kid2.op == REOP_FLAT && result.kid.lowSurrogate == 0 && result.kid2.lowSurrogate == 0) {
    ...
    private static boolean parseIdentityEscape(CompilerState state, ParserParameters params) {
        ...
        if (state.cp < state.cpend) {
            ...
            // if not unicode but named capture groups, any source charatcter and c and k are not allowe
            if (params.unicodeMode) {
                // case ^ $ \ . * + ? ( ) [ ] { } | / : true
                // default false
                switch (c) {
    ...
    private static boolean parseCharacterAndCharacterClassEscape(
        ...
        switch (c) {
            case '0':
                // in non-unicode mode, if next character is a decimal digit, then it must be an octal escape
                if (state.cp < state.cpend
                        && isDigit(src[state.cp])) {
                    if (params.unicodeMode) {
                        reportError("msg.invalid.escape", "");
                        return false;
                    } else {
    ...
    static class ClassContents {
        ...
        ArrayList<String> unicodeProperties = new ArrayList<String>();
    }
            ...
            if (src[state.cp] == '\\') {
                ...
                if (state.cp < state.cpend && src[state.cp] == 'b') {
                    ...
                    if (!parseCharacterAndCharacterClassEscape(state, params)) {
                        if (src[state.cp] == 'c' && !params.unicodeMode) { // when lookahead=c, parse the \\ as a literal
                            thisCodepoint = (char) '\\';
                        } else {
                            reportError("msg.invalid.escape", "");
                            return null;
                        ...
                        if (state.result.op == REOP_FLAT) {
                            if (state.result.lowSurrogate == 0) {
                                ...
                                thisCodepoint = Character.toCodePoint(state.result.chr, state.result.lowSurrogate);
                            }
                        } else {
        ...
        while (t != null) {
            ...
            switch (t.op) {
                ...
                case REOP_NAMED_BACKREF:
                    {
                        //TODO: Handle bounds
                        String backRefName = re.namedBackReferences.get(t.namedCaptureGroupBackRefIndex);
    ...
    private static int classMatcher(REGlobalData gData, RECharSet charSet, int codePoint) {
        ...
        if (codePoint <= 0xFFFF) {
            ...
            if ((charSet.length == 0
                    ...
                    || (charSet.bits[byteIndex] & (1 << (codePoint & 0x7))) == 0) ^ charSet.classContents.sense)
                return 1;
        } else {
            ...
            if (!hasCodePoint ^ charSet.classContents.sense) {
                ...
                for (int i = 0; i < charSet.classContents.nonBMPRanges.size(); i += 2) {
                    if (codePoint >= charSet.classContents.nonBMPRanges.get(i)
                        ...
                        if (!charSet.classContents.sense) {
                            return Character.charCount(codePoint);
                        }
                    }
                }
            ...
            case REOP_NAMED_BACKREF:
                {
                    ...
                    pc += INDEX_LEN;
                    if (gData.parens == null || backRefNameIndex >= gData.regexp.namedBackReferences.size()) {
                        break;
                    }
            ...
            case REOP_UCFLAT1i:
                {
                    ...
                    pc += INDEX_LEN;
                    if (cpInBounds && foldCase(input.codePointAt(cpToMatch)) == foldCase(codepointToMatch)) {
                        result = true;
                        gData.cp += cpDelta;
                    }
                }
            ...
            case REOP_NCLASS:
                {
                    ...
                    pc += INDEX_LEN;
                    if (cpInBounds) {
                        int codepointCharCount = classMatcher(
                                gData, gData.regexp.classList[index], input.codePointAt(cpToMatch));
                    ...
                    case REOP_ALTPREREQ2:
                        {
                            ...
                            if (op == REOP_ALTPREREQ2) {
                                if (c != matchCh1
                                        && classMatcher(
                                                gData, gData.regexp.classList[matchCh2], c) < 0) {
                                    result = false;
                                    break;
                                }
                            } else {
                    ...
                    if (inUnicodeMode) {
                        ...
                        if (codepoint == anchorCh ||
                                ((gData.regexp.flags & JSREG_FOLD) != 0
                                        && foldCase(codepoint) == foldCase(anchorCh)))
                            break;
                    } else {
