...
/**
 * Tests for {@link ScriptRuntime#same(Object, Object)}, the SameValue comparison implemented by
 * Object.is. JS numbers may be boxed as different {@link Number} subtypes (or as {@link
 * BigInteger}), so the comparison must be by numeric value rather than by {@link
 * Object#equals(Object)}. It must treat all NaNs as identical, distinguish +0 from -0, and reject
 * comparisons between values of different primitive types.
 */
public class ScriptRuntimeSameTest {
    ...
    private static Stream<Arguments> pairs() {
        ...
        return Stream.of(
                ...
                // SameValue distinguishes +0 from -0, across all Number subtypes.
                ...
                Arguments.of(-0.0, 0L, false),
                Arguments.of(0.0, (byte) 0, true),
                Arguments.of(-0.0, (byte) 0, false),
                Arguments.of(0.0, (short) 0, true),
                Arguments.of(-0.0, (short) 0, false),
                Arguments.of(0.0, 0.0f, true),
                Arguments.of(-0.0, 0.0f, false),
                Arguments.of(0.0f, -0.0f, false),
    ...
    @ParameterizedTest(name = "same({0}, {1}) == {2}")
    @MethodSource("pairs")
    void same(Object x, Object y, boolean expected) {
        try (var ignored = Context.enter()) {
            assertEquals(
                    expected,
                    ScriptRuntime.same(x, y),
                    () -> "same(" + describe(x) + ", " + describe(y) + ")");
            assertEquals(
                    expected,
                    ScriptRuntime.same(y, x),
                    () -> "same(" + describe(y) + ", " + describe(x) + ")");
        }
    }
